...
(3 rows)
```
Новой записи нет, потому что в данном типе транзакций будут видны только те строки, которые были видны на момент начала транзакции. Пока транзакция открыта, другие транзакции не могут изменять захваченные строки, но могут добавлять новые, которые текущая транзакция не увидит.
...
Новой записи нет, потому что транзакция всё ещё продолжается и видит только те данные, которые были на момент начала транзакции.
...
Новая запись появилась, потому что началась новая транзакция, и в этот момент видны все изменения, совершённые до её начала.
