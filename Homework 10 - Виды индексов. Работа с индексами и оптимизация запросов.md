...
Создадим индекс на год альбома:
```
...
Посмотрим план запроса альбомов за 2012 год:
```sql
explain (select Name, Genre from albums where Year = 2012);
```
...
```
                                   QUERY PLAN                                   
--------------------------------------------------------------------------------
 Bitmap Heap Scan on albums  (cost=6.87..154.03 rows=333 width=72)
   Recheck Cond: (year = 2012)
   ->  Bitmap Index Scan on idx_albums_year  (cost=0.00..6.78 rows=333 width=0)
         Index Cond: (year = 2012)
(4 rows)
```
Выбран Bitmap Scan. Он включает:
...
Добавим столбец, наполним вектором, созданным из поля Text:
```
alter table songs add column TextVector tsvector;
```
В консоль выведется:
```
ALTER TABLE
...
Наполним новый столбец данными:
```
update songs set TextVector = to_tsvector('russian', Text);
```
В консоль выведется:
```
UPDATE 100005
...
Попробуем найти песню "Ария - Осколок льда" по фразе "я любил":
```
select Text from songs where TextVector @@ phraseto_tsquery('russian', 'Я любил');
```
...
(2 rows)
```
Это связано с тем, что часто встречающиеся слова не сохраняются в вектор, а однокоренные слова часто записываются одинаково (но не всегда, например "знаем" сохраняется как "зна", а "знает" как "знает").  
От слов "любил" и "люблю" в векторе остаётся "люб", поэтому эти две песни подходят под критерии поиска.

Теперь посмотрим на оценку запроса без индекса:
```
...
```
                        QUERY PLAN                        
----------------------------------------------------------
 Seq Scan on songs  (cost=0.00..5593.06 rows=22 width=37)
   Filter: (textvector @@ '''люб'''::tsquery)
(2 rows)
```
Добавим индекс полнотекстового поиска и оценим запрос:
```sql
create index idx_songs_text_search on songs using gin(TextVector);
```
В консоль выведется:
```
CREATE INDEX
...
```
                                      QUERY PLAN                                      
--------------------------------------------------------------------------------------
 Bitmap Heap Scan on songs  (cost=16.17..99.75 rows=22 width=37)
   Recheck Cond: (textvector @@ '''люб'''::tsquery)
   ->  Bitmap Index Scan on idx_songs_text_search  (cost=0.00..16.16 rows=22 width=0)
         Index Cond: (textvector @@ '''люб'''::tsquery)
(4 rows)
```
Как видно, теперь запрос выполняется более оптимально, используя индекс вместо последовательного сканирования.
...
Для начала посмотрим на первых пяти записях примерное количество слов в каждой песне (в остальных будет только одно автосгенерированное слово):
```sql
select length(coalesce(Text, '')) - length(replace(coalesce(Text, ''), ' ', '')) + 1 from songs;
```
В консоль выведется:
```
 ?column? 
...
Добавим индекс:
```sql
create index ind_songs_text_words_count on songs((length(coalesce(Text, '')) - length(replace(coalesce(Text, ''), ' ', '')) + 1));
```
В консоль выведется:
```
CREATE INDEX
...
Будем создавать индекс на год + жанр альбома.  
Для начала проверим работу без индекса:
```sql
explain select * from albums where year = 1989 and genre = 'русская поп-музыка';
```
...
```
                                  QUERY PLAN                                   
-------------------------------------------------------------------------------
 Index Scan using idx_albums_year on albums  (cost=0.29..8.30 rows=1 width=84)
   Index Cond: (year = 1989)
   Filter: ((genre)::text = 'русская поп-музыка'::text)
(3 rows)
```

Добавим индекс:
```sql
create index ind_albums_year_and_genre on albums(year, genre);
```
В консоль выведется:
```
CREATE INDEX
...
Посмотрим, используется ли индекс:
```sql
explain select * from albums where year = 1989 and genre = 'русская поп-музыка';
```
...
```
                                       QUERY PLAN                                        
-----------------------------------------------------------------------------------------
 Index Scan using ind_albums_year_and_genre on albums  (cost=0.29..8.30 rows=1 width=84)
   Index Cond: ((year = 1989) AND ((genre)::text = 'русская поп-музыка'::text))
(2 rows)
```

Как видим, теперь используется один из наиболее эффективных методов сканирования - Index Scan.
...
Описание действий приведено выше, столкнулся со следующими проблемами:
* Трудно подобрать пример для полнотекстового поиска, т.к. некоторые однокоренные слова воспринимаются разными, некоторые вовсе пропускаются. Задача подобрать две фразы из песен с похожим текстом не такая простая, как казалось на первый взгляд.
* Если с первого раза не получается сгенерировать данные, транзакция откатывается, а автоинкремент уходит дальше, что не даёт ориентироваться на заранее заданные индексы, и нужно либо удалять таблицы и создавать данные заново, либо запрашивать идентификаторы альбомов по их названию, прежде чем добавлять песню.
